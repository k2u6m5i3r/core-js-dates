...
function getTime(date) {
  ...
  return `${dateHour.padStart(2, '0')}:${dateMin.padStart(2, '0')}:${dateSec.padStart(2, '0')}`;
}
...
function getNextFriday(date) {
  ...
  const indexYahr = date.getFullYear();

  let flag = true;
  const indexMonth = month;
  let indexDay = day;
  for (indexDay = day; flag; indexDay += 1) {
    if (
      new Date(indexYahr, indexMonth, indexDay).getDay() === 5 &&
      indexDay !== day
    ) {
      flag = false;
    }
  }
  return new Date(Date.UTC(indexYahr, indexMonth, indexDay - 1, 0, 0, 0, 0));
}
...
function isDateInPeriod(date, period) {
  ...
  const stopDate = new Date(period.end).getTime();
  const dateEvent = new Date(date).getTime();
  return dateEvent >= startDate && dateEvent <= stopDate;
}
...
function formatDate(date) {
  ...
  const yahr = dt.getUTCFullYear();
  return new Date(yahr, month, day, hour, minute, second).toLocaleString(
    'en-US'
  );
}
...
function getCountWeekendsInMonth(month, year) {
  let indexDay = 1;
  const indexMaxDay = 32 - new Date(year, month - 1, 32).getDate();
  const indexMonth = month - 1;
  const indexYahr = year;
  let ans = 0;
  for (; indexDay <= indexMaxDay; indexDay += 1) {
    const currentDay = new Date(indexYahr, indexMonth, indexDay);
...
function getWeekNumberByDate(date) {
  ...
  const deltaDay =
    new Date(date.getFullYear(), 0, 1).getDay() === 0
      ? 6
      : new Date(date.getFullYear(), 0, 1).getDay() - 0.5;
  const startDateMillis =
    new Date(date.getFullYear(), 0, 1).getTime() - deltaDay * MILLIS_TO_DAY;
  const endDateMillis = date.getTime();
  return Math.ceil(
    (endDateMillis - startDateMillis + 43200000) / MILLIS_TO_DAY / 7
  );
}
...
function getNextFridayThe13th(date) {
  ...
  let indexYahr = date.getFullYear();

  for (; flag; indexMonth += 1) {
    if (indexMonth === 11) {
      indexMonth = 0;
      indexYahr += 1;
    }
    if (new Date(indexYahr, indexMonth, 13).getDay() === 5) {
      flag = false;
    }
  }
  return new Date(indexYahr, indexMonth - 1, 13);
}
...
function isLeapYear(date) {
  const indexYahr = date.getFullYear();
  return (
    (indexYahr % 4 === 0 && indexYahr % 100 !== 0) || indexYahr % 400 === 0
  );
}
